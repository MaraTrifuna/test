...
With Faunus it is possible to connect to use Titan as the source of a "Hadoop":hadoop.apache.org MapReduce job chain. Like Titan, Faunus uses Gremlin as its traversal/query language. However, Faunus compiles its Gremlin statements down to MapReduce jobs. Assume the Titan/Cassandra maintains _The Graph of the Gods_. A Faunus/Gremlin shell can be used to create a @FaunusGraph@ which uses Titan/Cassandra as the source of the input data into the Hadoop job.
