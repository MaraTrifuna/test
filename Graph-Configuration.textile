...
| fast-property | Whether all properties should be pre-fetched for a vertex with the first property lookup on that vertex. Significantly speeds up multiple property retrievals. Disable on graphs with vertices that have lots of properties.  | boolean | _true_ | yes |
