...
|_. Option |_. Description |_. Value |_. Default |_. Modifiable |
| cache.db-cache | Enables the database level cache | _true_ or _false | _false_ | yes |
...
| fast-property | Whether all properties should be pre-fetched for a vertex with the first property lookup on that vertex. Significantly speeds up multiple property retrievals. Disable on graphs with vertices that have lots of properties.  | boolean | _true_ if storage backend is distributed else _false_ | yes |
...
* [[Hazelcast Configuration | Using Hazelcast]]
* [[Persistit Configuration | Using Persistit]]
