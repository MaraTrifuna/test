...
It is possible to use Blueprints' "IdGraph":https://github.com/tinkerpop/blueprints/wiki/Id-Implementation with Titan. However, if you want to use IdGraph for user specified ids on ~both~ vertices and edges, then it is important to declare the @__id@ index using @TitanGraph.makeType()@ first in Titan:

```text
gremlin> g
==>titangraph[local:/tmp/test]
gremlin> g.makeType().name('__id').unique(OUT).indexed(Vertex.class).indexed(Edge.class).dataType(String.class).makePropertyKey()
==>v[36028797018964170]
