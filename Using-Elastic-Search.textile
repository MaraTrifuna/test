...
[[images/titan-elasticsearch.png|width=175px|align=right|float]]
...
Please see the [[Version Compatibility]] chart for details on what versions of ES will work with Titan.
...
h2. Elasticsearch Remote Configuration
...
| sniff | Whether client transport sniffing is enabled. When encountering connection problems, in particular on AWS, disable this option | boolean | _true_ | yes |

h2. Troubleshooting

h3. Connection Issues to remote Elasticsearch cluster
...
h2. Optimizing Elasticsearch

h3. Write Optimization

For [[bulk loading]] or other write-intense applications, consider increasing Elasticsearch's refresh interval. Refer to "this discussion":https://groups.google.com/d/topic/elasticsearch/yp6bTiP2JYE/discussion on how to increase the refresh interval and its impact on write performance. Note, that a higher refresh interval means that it takes a longer time for graph mutations to be available in the index.
...
* Check out [[example graph configurations|Example Graph Configuration]] for complete configurations including the storage backend.