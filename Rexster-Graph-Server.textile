...
If not using [[Titan Server]], Rexster must first be downloaded. Rexster can be downloaded at "this location":https://github.com/tinkerpop/rexster/wiki/Downloads. Below is a snippet of shell commands that demonstrate the process for getting Rexster downloaded and started using a default configuration.
...
_NOTE_ - For those using ElasticSearch, Rexster includes lucene-core-3.5.0.jar (a neo4j dependency).  Assuming Neo4j is not being utilized in the Rexster instance, simply delete that file from the rexster @lib@ directory.  Removing it will eliminate a conflict with Titan and allow ElasticSearch to start.  
