...
* Extend with more REST endpoints by adding "kibbles":https://github.com/tinkerpop/rexster/wiki/Creating-an-Extension to Rexster.
...
Use @CNTRL-C@ to kill the server. Now that Rexster is downloaded and working, it is necessary to configure Rexster to work with Titan.
...
```
*NOTE*: The graph-location value @db/titan@ refers to the graph created in the "Getting Started":https://github.com/thinkaurelius/titan/wiki/Getting-Started page. The only difference is that the files have been moved from @/tmp/titan@.

[[https://github.com/tinkerpop/rexster/raw/master/doc/images/rexster-character-2.png|float=right|width=100px]]
There are three ways to configure Rexster for use with Titan:

# If building Titan and Rexster from source, copy the @target/titan-0.1-SNAPSHOT-jar-with-dependencies.jar@ to the Rexster @./rexster-server/src/main/ext@ directory and then do @mvn clean install@ on Rexster.
# If building Titan from source and using Rexster distribution, copy the @target/titan-0.1-SNAPSHOT-jar-with-dependencies.jar@ to the Rexster @./rexster-server/target/rexster-server-${VERSION}/ext@ directory.
...
*NOTE*: Future releases (beyond the 0.1-alpha) will have a much more efficient/easier packaging model.
...
Rexster has a rich "API":https://github.com/tinkerpop/rexster/wiki/Basic-REST-API for working with Blueprints Graph implementations, but there are some limitations to consider when using Rexster with Titan.  These limitations are driven by the [[limitations|Blueprints Interface]] in the Titan Blueprints implementation.

* The POST of an edge cannot be done with an identifier.  In other words, do this: @http://localhost/graphs/titan/edges@ and _not_ this @http://localhost/graphs/titan/edges/1234@.
* PUT/DELETE of an existing edge is not supported.
* All operations on the @indices@ resource are not supported.  This resource refers to manual indices.  Key indices are supported.