...
This section explains how to configure Rexster to use a Titan graph database. These instructions assume that the a graph has been created in Titan as described in the "Loading Data Into Titan" section of the [[Getting Started]] page. An example @rexster.xml@ file is provided below (see "Rexster Configuration":https://github.com/tinkerpop/rexster/wiki/Rexster-Configuration).
...
```text
cp TITAN_HOME/target/titan-x.y.z-standalone/lib/*.* ~/jvm/rexster-server-x.y.z/ext/titan
```

or, if using the Titan zipped distribution download:

```text
cp TITAN_HOME/lib/*.* ~/jvm/rexster-server-x.y.z/ext/titan
```
