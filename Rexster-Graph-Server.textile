...
This section explains how to configure Rexster to use a Titan graph database. These instructions assume that the a graph has been created in Titan as described in the "Loading Data Into Titan" section of the [[Getting Started]] page. 
...
```text
cp TITAN_HOME/target/titan-x.y.z-standalone/lib/*.* REXSTER_HOME/rexster-server-x.y.z/ext/titan
```

or, if using the Titan zipped distribution download:

```text
cp TITAN_HOME/lib/*.* REXSTER_HOME/rexster-server-x.y.z/ext/titan
```
