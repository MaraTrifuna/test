...
"Gremlin":http://gremlin.tinkerpop.com is a graph traversal language that works for any "Blueprints":http://blueprints.tinkerpop.com enabled graph engine. Gremlin is a functional language whereby traversal operators are chained together. Data (e.g. a vertex) is inserted into the "left" of the function chain and data (e.g. property values) is emitted from the "right" of the function chain. This section of documentation will only briefly overview Gremlin. For more complete documentation on Gremlin, please see the "online documentation":http://gremlin.tinkerpop.com. Moreover, the examples are with respects to Gremlin-Groovy and note that there are other "JVM language implementations of Gremlin":https://github.com/tinkerpop/gremlin/wiki/JVM-Language-Implementations.
...
When chaining together functions, the incoming type must match the outgoing type, where @U@ matches anything. Thus, the "co-battled" is correct.