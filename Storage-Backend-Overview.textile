...
All of the storage backends shown above persist the data durably to disk. For very large graphs, it is beneficial to use relatively inexpensive disk storage for old or stale data. However, disk access has high latencies and limits throughput.

For low latency applications, Titan additionally supports in-memory storage backends. Those hold all data in-memory and thereby avoid the additional complexities and access cost of disks.
...
| [[Hazelcast|Using Hazelcast]] | _hazelcastcache_ | transactional consistency | highly available | linear scalability | yes | in-memory |
