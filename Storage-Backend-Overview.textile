...
However, the "CAP theorem":http://en.wikipedia.org/wiki/CAP_theorem stipulates that any practical database can only provide 2 of the 3 desirable properties: *C*onsistency, *A*vailability, and *P*artitionability (i.e. scalability). The choice of storage backend is therefore a tradeoff guided by the requirements of a particular use case.
