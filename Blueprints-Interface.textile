...
Edge marriedTo = graph.addEdge(null, jupiter, juno, "marriedTo");
```

In the code example above, two vertices were created named "Jupiter" and "Juno." These two vertices were joined in holy matrimony via a @marriedTo@ edge.

h2. Vertex Edge Queries

!https://github.com/thinkaurelius/titan/raw/master/doc/images/super-node.png!

Blueprints supports the notion of vertex-centric queries which can be understood as a query of the edges directly incident to a vertex. An example vertex query is provided below:
