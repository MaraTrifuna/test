...
Key indices must be defined before that key is being used. In other words, the first reference to the key name must be inside the @Graph.createKeyIndex()@ method. If the key has been previously used, creating a key index for that key after the fact will throw an exception. Once a key index has been created it cannot be dropped. These limitations are temporary.

Titan only supports vertex key indices, i.e. @Graph.createKeyIndex("name",Vertex.class)@.

h3. Transactional Scope

Titan strictly enforces the transactional scope, which means that elements created within a transaction are invalid once that transaction is stopped. Operations on invalid elements - other than @getId()@ - may cause an exception. This is done to ensure that operations are not executed on out-of-date elements which could lead to inconsistencies.
This is not a deviation from the Blueprints transactional semantic but important to note for users of other graph database frameworks.