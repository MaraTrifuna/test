...
The Gremlin terminal is a "Groovy":http://groovy.codehaus.org/ shell. Groovy is a superset of Java that has various shorthand notations that make interactive programming easier. Likely-wise Gremlin is a superset of Groovy with various shorthand notations that make graph traversals easy to express. The basic examples below demonstrate handling numbers, strings, and maps. The remainder of the tutorial will discuss graph-specific constructs.
...
*NOTE*: Please refer to "GremlinDocs":http://gremlindocs.com for a easy to use reference on the Gremlin graph traversal language.
...
The @GraphOfTheGodsFactory.create()@ method does the following to the newly constructed graph prior to returning it:
  ...
  # Generate a collection of global and vertex-centric indices on the graph.
...
A typical pattern for accessing data in a graph database is to first locate the entry point into the graph using a graph index and then, traverse from those elements (vertices/edge) to other elements in the graph via the explicit graph structure.

Given that there is a unique index on @name@ property, the Saturn vertex can be retrieved.  The property map (i.e. the key/value pairs of Saturn) can then be examined. As demonstrated, the Saturn vertex has a @name@ of "saturn," an @age@ of 10000, and a @type@ of "titan." The grandchild of Saturn can be retrieved with a traversal that expresses: "Who is Saturns' childs' child?" (the inverse of "father" is "child"). The result is Hercules.
...
The property @place@ is also in a graph index. The property @place@ is an edge property. Thus, Titan can index edges in a graph index. This means it is possible to query the graph for all events that have happened within 50 kilometers of "Athens":http://en.wikipedia.org/wiki/Athens (latitude:37.97 and long:23.72). Then, given that information, which vertices were involved in the event.
...
Graph indices form on half of the indexing model of Titan. Graph indices are accessible via the @Graph.query()@ method. The other indexing model is vertex-centric indices which are described later. Vertex-centric indices are accessible via the @Vertex.query()@ method.
...
Because @time@ is a vertex-centric index, retrieving edges incident to Hercules according to a contraint/filter on @time@ is faster (typically @O(log n)@, where @n@ is the number incident edges) than doing a linear scan of all edges and filtering. Gremlin is smart enough to use vertex-centric indices when available. A @toString()@ of a Gremlin expression shows the underlying "pipeline":http://pipes.tinkerpop.com construction.
...
Finally, Pluto's life in Tartarus is because he shows no concern for death. His brothers, on the other hand, chose their locations based upon their love for certain qualities of those locations.

```text
gremlin> g.query().has('reason',Text.CONTAINS,'love').edges()
==>e[2B-g-2F0LaTPQBU][16-lives->8]
==>e[2H-k-2F0LaTPQBU][20-lives->12]
gremlin> g.query().has('reason',Text.CONTAINS,'love').edges().collect{
...
This section presented some basic examples of how query and traverse _The Graph of the Gods_ in Titan. In essence, a graph database is all about representing some world model (structure) and traversing it to solve problems (process). 
