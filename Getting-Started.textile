...
This page will make use of the following toy graph deployed with Titan. The toy graph is called _The Graph of the Gods_ and it provides a collection of heterogenous vertices and edges that outline the relationships between the beings and places within the Roman pantheon and its ancient saga. The raw _The Graph of the Gods_ "GraphML":http://graphml.graphdrawing.org/ file is available at "this location":https://github.com/thinkaurelius/titan/blob/master/data/graph-of-the-gods.xml.
...
Titan can be downloaded from the "Downloads":https://github.com/thinkaurelius/titan/downloads section. Once downloaded and unpacked, a "Gremlin":http://gremlin.tinkerpop.com terminal can be started. Commands can now be issued to Titan.
...
First, it is important to import the @TitanFactory@ class. @TitanFactory@ provides methods to create various Titan instances (e.g. embedded, distributed, etc.). Second, a local embedded instance of Titan can be created using the @TitanFactory.open(String directory)@ method. A Titan database has been created.
