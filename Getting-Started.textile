...
gremlin> g.query().has('place',Geo.WITHIN,Geoshape.circle(37.97,23.72,50)).edges().collect { it.bothV.name.next(2) }
==>[hercules, hydra]
==>[hercules, nemean]
```
...
Because @time@ is a vertex-centric index, retrieving edges incident to Hercules according to a contraint/filter on @time@ is faster (typically @O(log n)@, where @n@ is the number incident edges) than doing a linear scan of all edges and filtering.
