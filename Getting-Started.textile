...
[[images/graph-of-the-gods-2.png|width=700px|align=center]]
...
[[images/titan-head.png|align=right|float|width=115px]]
...
Titan can be downloaded from the "Downloads":https://github.com/thinkaurelius/titan/wiki/Downloads section of the project repository (get the @titan-all@ version). Once retrieved and unpacked, a "Gremlin":http://gremlin.tinkerpop.com terminal can be started. The Gremlin "REPL":http://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop (i.e. interactive shell) is distributed with Titan and differs slightly from the main "TinkerPop":http://tinkerpop.com Gremlin distribution in that is comes preloaded with Titan-specific @imports@ and helper methods. In the example below, @titan.zip@ is used, however, be sure to unzip the zip-file that was downloaded.
...
==>[GremlinStartPipe, VertexQueryPipe(out,[battled],has,edge), IdentityPipe, InVertexPipe, PropertyPipe(name)]
```
