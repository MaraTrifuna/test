...
Finally, for developing against a Titan SNAPSHOT version, be sure to add the following repository to the @pom.xml@. For stable releases, no repository is required as stable releases are deployed to "Apache's Central Repository":http://search.maven.org/.
