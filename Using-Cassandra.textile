...
!images/titan-modes-embedded.png!
...
| storage.cassandra.thrift.frame_size_mb | The maximum frame size to be used by thrift for transport. Increase this value when retrieving very large result sets. _Only applicable when storage.backend=cassandrathrift_ | Integer | 16 | No |
...
Titan over Cassandra supports global vertex and edge iteration. However, note that all these vertices and/or edges will be loaded into memory which can cause @OutOfMemoryException@. Use "Faunus":http://thinkaurelius.github.com/faunus/ to iterate over all vertices or edges in large graphs.
...
* SSH into any Cassandra instance node: <code>ssh -i [your-private-key].pem ubuntu@[public-dns-name-of-any-cassandra-instance]</code>
...
* SSH into the newly created instance via <code>ssh -i [your-private-key].pem ec2-user@[public-dns-name-of-the-instance]</code>. You may have to wait a little for the instance to launch.

* "Download":https://github.com/thinkaurelius/titan/wiki/Downloads the current Titan distribution with @wget@ and unpack the archive locally to the home directory. Start the gremlin shell to verify that Titan runs successfully. For more information on how to unpack Titan and start the gremlin shell, please refer to the "Getting Started guide":Getting-Started.
...
h3. Connect to Cassandra cluster in EC2 from outside EC2
