...
For example, suppose we have a running Cassandra cluster where one of the machines has the IP address 77.77.77.77 (comma separate to add more machines to the connection options), then connecting Titan with the cluster is accomplished as follows:
