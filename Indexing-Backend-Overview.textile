...
The @Compare@ enum specifies the comparison predicates used for index query construction and used in the examples above:
* EQUAL
* NOT_EQUAL
* GREATER_THAN
* GREATER_THAN_EQUAL
...
The @Text@ enum specifies the full-text predicates used to query for text that contains words such that:
* CONTAINS: matches the entire word
* PREFIX: matches the beginning of the word
* REGEX: the given regular expression matches the entire word

The @Geo@ enum specifies the geo-location predicate @Geo.CONTAINS@ which holds true if one geometric object contains the other.
...
h4. Full-text search is tokenized

Full-text search operates on individual words and hence the query must be tokenized as well. Querying for multiple words requires adding multiple has clauses.

h4. OrderBy keys must be indexed

Queries which contain an orderBy clause can only be executed efficiently against an external indexing backend if the orderBy key is indexed as well. This is important in cases where the orderBy key is different from the query keys.
