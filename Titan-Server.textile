[[images/titan-server-embedded.png
|width=200px|align=right|float]]

Titan Server is a light-weight server that manages remote connections to Titan and thereby allows applications to execute [[Gremlin|Gremlin Query Language]] queries against a Titan graph cluster. Titan Server embeds Rexster's [[HTTP/REST|https://github.com/tinkerpop/rexster/wiki/Basic-REST-API]] Server and [[RexPro|https://github.com/tinkerpop/rexster/wiki/RexPro]] Server, a light-weight component of the "Rexster":http://rexster.tinkerpop.com Server which can efficiently handle hundreds of database connections. Titan Server bundles all the features and benefits of Rexster with Titan.
...
# A configuration file for the HTTP/REST and RexPro server. Titan Server can be configured with either one running or both.  Refer to the "Rexster configuration documentation":https://github.com/tinkerpop/rexster/wiki/Rexster-Configuration for more information. This first argument is optional and can be omitted, in which case default configurations are used for both HTTP/REST and RexPro.  To configure Titan Server to use one one server or the other, simply omit the appropriate section from the configuration file.  Note that there is a sample Rexster configuration file in @config/titan-server-rexster.xml@.
...
h3. Connecting to Titan Server Via RexPro

The client application can now connect to the Titan Server on the configured hostname and port to issue Gremlin queries using [[RexsterClient|https://github.com/tinkerpop/rexster/wiki/RexPro-Java]].
...
h3. Connecting to Titan Server Via HTTP/REST

Titan Server will expose the same [[REST API|https://github.com/tinkerpop/rexster/wiki/Basic-REST-API]] as Rexster.
...
It is also possible to deploy "Rexster Extensions":https://github.com/tinkerpop/rexster/wiki/Extensions in Titan Server.  "Create":https://github.com/tinkerpop/rexster/wiki/Creating-an-Extension an extension and put it in Titan Server's path (i.e. the @lib@ directory).  Since Titan Server only exposes a single graph, it assumes that all extensions should be automatically deployed and be made available, therefore no specific configuration is required to expose the extension.  If the extension requires specific "configuration":https://github.com/tinkerpop/rexster/wiki/Extension-Configuration for its operations, simply add the configuration to an @<extensions>@ element within the root of the Rexster configuration file.

*Note* - Titan Server does not support "Dog House":https://github.com/tinkerpop/rexster/wiki/The-Dog-House.  It is always configured to be disabled.

h3. Connecting to Titan via Rexster Console
