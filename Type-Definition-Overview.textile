...
| @unique(Direction)@ | Configures the type to be unique in the given direction and acquires locks to ensure consistency | Label and Key | not functional | @TitanType.isUnique(Direction)@ |
| @unique(Direction, UniquenessConsistency)@ | Configures the type to be unique in the given direction with the specified consistency guarantee (either @LOCK@ or @NO_LOCK@). | Label and Key | not functional | @TitanType.isUnique(Direction)@ |
