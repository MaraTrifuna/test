...
* An edge lable is out-unique, if a vertex has at most one outgoing edge for that label. _father_ is an exmaple of an out-unique edge label, since each god has at most one father.
...
As before, @TitanTypes@ used in the signature must be either out-unique property keys or out-unique, unidirected edge lables.
