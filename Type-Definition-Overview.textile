...
Also, note some of the [[limitations and gotchas | Gotchas and Limitations ]] when it comes to data types.

The data type of an existing property can be retrieved as @TitanKey.getDataType()@.

h3. indexed(Element,index)

Registers the property key with the given index for the provided element type, i.e. vertex or edge. Depending on the specified element type, all vertices or edges with a property of this key are indexed against the given index by their property value. This allows vertices and/or edges to be retrieved by property value using @GraphQuery@ as described in [[Index Backend Overview]].

The name of the index must match one of the registered external indexing backends. External indexing backends are registered by name in the Titan configuration as described in [[Index Backend Overview]].
...
```java
g.makeKey("name").dataType(String.class).indexed(Vertex.class).indexed(Edge.class).indexed(Vertex.class,"search").indexed(Edge.class,"search").make()
```
...
h3. Single and multi-value Keys
...
h3. sortKey(TitanType…) and signature(TitanType…)

Specifying the sort key of a label allows edges with this label to be efficiently retrieved in the specified sort-order. Titan builds [[vertex-centric indexes | ??]] for each label according to the sort key definition which can significantly speed up queries.
...
Note, that @TitanTypes@ used in the primary key must be single-valued property keys or many-to-one, uni-directed edge labels.
