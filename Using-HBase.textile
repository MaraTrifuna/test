...
Finally, if an HBase cluster has already been deployed and more memory is required of the master or regionservers, simply edit the @$HBASE_HOME/conf/hbase-env.sh@ files on the respective machines with requisite @-Xmx -Xms@ parameters. Once edited, stop/start the master and/or regionservers as described previous.