...
!images/titan-modes-distributed.png!
...
!images/titan-modes-rexster.png!
...
| storage.short&#8209;cf&#8209;names | True to replace Titan's conventional column family names by single character mnemonics.  "Short CF names save space because HBase stores the CF in every KeyValue.":http://hbase.apache.org/book/rowkey.design.html#keysize.cf | Boolean | false in 0.4.x, true in 0.5.x | No |
...
Titan over HBase supports global vertex and edge iteration. However, note that all these vertices and/or edges will be loaded into memory which can cause @OutOfMemoryException@. Use "Faunus":http://thinkaurelius.github.com/faunus/ to iterate over all vertices or edges in large graphs.
...
h2. Targeting Hadoop 2

Titan's HBase storage backend depends on the Apache HBase 0.94 Maven artifact.  This in turn depends on Hadoop 1.0.  However, "HBase supports multiple Hadoop versions and recommends Hadoop 2":http://hbase.apache.org/book/configuration.html#hadoop.

Titan is supported with its released version of HBase and the associated default Hadoop version (see the "version compatibility matrix":https://github.com/thinkaurelius/titan/wiki/Version-Compatibility).  Due to the sheer number of interoperable HBase and Hadoop versions, Titan does not support every possible combination.  However, Titan attempts to maintain as wide a version compatibility range as possible by using only a minimal set of HBase API classes and methods.  Titan does not depend on Hadoop directly and is only exposed to it indirectly through HBase.  Furthermore, since version 0.4.1, Titan includes Maven build options to make targeting Hadoop 2.0 or 2.2 easier.  It should be possible to target any recent Hadoop version and compatible HBase version without modifying Titan's source code, though targeting Hadoop versions besides 1.0, 2.0, or 2.2 may require tweaking dependency versions in <code>titan-hbase/pom.xml</code>.
...
h3. Rebuilding HBase with Hadoop 2

In this subsection, we download an HBase 0.94.12 release tarball and compile it against Hadoop 2.2.0.

# Download HBase 0.94.12 from the closest mirror on this page: "http://www.apache.org/dyn/closer.cgi/hbase/hbase-0.94.12/hbase-0.94.12.tar.gz":http://www.apache.org/dyn/closer.cgi/hbase/hbase-0.94.12/hbase-0.94.12.tar.gz
...
# Rebuild Titan-Core and Titan-Test
## <code>pushd titan-core; mvn clean install -DskipTests=true; popd</code>
## <code>pushd titan-test; mvn clean install -DskipTests=true; popd</code>
# Build Titan-HBase: <code>pushd titan-hbase; mvn clean install -Dhadoop.profile=2.0 -Dhadoop.version=2.2.0; popd</code>
