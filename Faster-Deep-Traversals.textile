Deep traversals on a graph often require executing identical vertex queries on sets of queries. For instance, retrieving all siblings of friends of some user vertex @v@ executes an identical vertex centric query for each of @v@ 's friends:
