...
Titan only supports iteration over all vertices and edges for local storage backends. For distributed storage backends, such iteration will likely cause @OutOfMemoryException@ and is therefore disabled. Use Faunus (coming soon) to implement global graph operations on distributed Titan graphs.
...
This pitfall constrains the graph schema. While the graph schema can be extended, previous declarations can not be changed. 
