...
Transactions are started automatically with the first operation executed against the graph. One does NOT have to start a transaction manually. The method @startTransaction@ is used to start [[multi threaded transactions]] only.
